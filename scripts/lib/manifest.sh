...
manifest_profile_phases() {
  ...
  awk '
    ...
    in_phases && /^    -[[:space:]]+/ { value=$0; sub(/^    -[[:space:]]+/, "", value); gsub(/["\047]/, "", value); print value }
  ' "$profile_file"
}

manifest_phase_field() {
  local phase_file="$1" field="$2"
  awk -v wanted_field="$field" '
    /^phase:[[:space:]]*$/ { in_phase=1; next }
    in_phase && /^[^[:space:]]/ { in_phase=0 }
    in_phase && $0 ~ "^  " wanted_field ":[[:space:]]*" { value=$0; sub("^  " wanted_field ":[[:space:]]*", "", value); gsub(/^["\047]|["\047]$/, "", value); print value; exit }
  ' "$phase_file"
}

manifest_validation_field() {
  local phase_file="$1" field="$2"
  awk -v wanted_field="$field" '
    /^validation:[[:space:]]*$/ { in_validation=1; next }
    in_validation && /^[^[:space:]]/ { in_validation=0 }
    in_validation && $0 ~ "^  " wanted_field ":[[:space:]]*" { value=$0; sub("^  " wanted_field ":[[:space:]]*", "", value); gsub(/^["\047]|["\047]$/, "", value); print value; exit }
  ' "$phase_file"
}
...
manifest_phase_steps() {
  ...
  awk '
    ...
    in_order && /^    -[[:space:]]+/ { value=$0; sub(/^    -[[:space:]]+/, "", value); gsub(/["\047]/, "", value); print value }
  ' "$phase_file"
}

manifest_step_field() {
  local phase_file="$1" step="$2" field="$3"
  awk -v wanted_step="$step" -v wanted_field="$field" '
    /^steps:[[:space:]]*$/ { in_steps=1; next }
    in_steps && /^[^[:space:]]/ { in_steps=0; in_step=0 }
    in_steps && /^  [^[:space:]][^:]*:[[:space:]]*$/ { key=$0; sub(/^  /, "", key); sub(/:[[:space:]]*$/, "", key); in_step=(key == wanted_step); next }
    in_step && $0 ~ "^    " wanted_field ":[[:space:]]*" { value=$0; sub("^    " wanted_field ":[[:space:]]*", "", value); gsub(/^["\047]|["\047]$/, "", value); print value; exit }
  ' "$phase_file"
}
