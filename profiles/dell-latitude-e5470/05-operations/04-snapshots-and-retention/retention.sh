...
command -v snapper >/dev/null || {
  printf '[ERROR] snapper is unavailable.\n' >&2
  exit 1
}
command -v jq >/dev/null || {
  printf '[ERROR] jq is unavailable.\n' >&2
  exit 1
}
snapper -c "$SNAPPER_CONFIG" get-config >/dev/null || {
  printf '[ERROR] Snapper config root is unavailable.\n' >&2
  exit 1
}
...
read -r confirmation
[[ "$confirmation" == "RETAIN" ]] || {
  printf '[ERROR] Retention was not authorized.\n' >&2
  exit 1
}
